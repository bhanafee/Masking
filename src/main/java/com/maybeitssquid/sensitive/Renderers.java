 ...
 *
 * <p>These factory methods are designed to be used in subclasses of {@link Sensitive} that override
 * {@link Sensitive#getRenderer()} to provide custom rendering behavior. The returned renderer
 * instances can be stored as static constants for efficient sharing across instances.
 *
...
public class Renderers {
  ...
  /**
   * Returns a renderer that truncates the beginning of the value, showing only the trailing
   * characters based on precision.
   *
   * <p>If precision is negative, shows the last half of the characters (rounded down). Otherwise,
   * shows at most {@code precision} trailing characters.
   *
   * @param <T> the type of CharSequence to render
  ...
  public static <T extends CharSequence> Renderer<T> truncate() {
    return (cs, p) ->
        cs == null ? "" : cs.subSequence(Renderers.redactions(p, cs.length()), cs.length());
  }

  /**
   * Returns a renderer that masks leading characters with the specified mask character.
   *
   * <p>If precision is negative, masks the first half of the characters (rounded up). Otherwise,
   * masks all but the last {@code precision} characters.
   *
   * @param <T> the type of CharSequence to render
  ...
  public static <T extends CharSequence> Renderer<T> mask(final char mask) {
    if (Character.isSurrogate(mask))
      throw new IllegalArgumentException(
          "Use code point to specify a mask value outside the Basic Multilingual Plane");
  ...
  /**
   * Returns a renderer that masks only characters matching the predicate, using the specified mask
   * character.
   *
   * <p>Characters not matching the predicate (such as delimiters) are preserved in place. The
   * precision applies only to matching characters. For example, masking an SSN "123-45-6789" with a
   * digit predicate and precision 4 would produce "###-##-6789".
   *
   * @param <T> the type of CharSequence to render
   ...
   */
  public static <T extends CharSequence> Renderer<T> mask(
      IntPredicate redactable, final int maskCodePoint) {
   ...
   */
  public static <T extends CharSequence> Renderer<T> mask(
      IntPredicate redactable, final char mask) {
    if (Character.isSurrogate(mask))
      throw new IllegalArgumentException(
          "Use code point to specify a mask value outside the Basic Multilingual Plane");
  ...
  /**
   * Returns a renderer that joins an array of CharSequences with a delimiter and applies the given
   * renderer.
   *
  ...
  public static <T extends CharSequence> Renderer<T[]> delimit(
      final int delimiterCodePoint, final Renderer<CharSequence> renderer) {
  ...
  /**
   * Returns a renderer that joins an array of CharSequences with a delimiter and applies a masking
   * renderer.
   *
   * @param <T> the element type of the CharSequence array
   * @param delimiterCodePoint the character to insert between array elements
   ...
   */
  public static <T extends CharSequence> Renderer<T[]> delimit(
      final int delimiterCodePoint, final int maskCodePoint) {
   ...
   */
  public static <T extends CharSequence> Renderer<T[]> delimit(
      final char delimiter, final char mask) {
    if (Character.isSurrogate(mask) || Character.isSurrogate(delimiter))
      throw new IllegalArgumentException(
          "Use code point to specify a mask or delimiter value outside the Basic Multilingual Plane");
    return delimit((int) delimiter, (int) mask);
  }

  /**
   * Returns a renderer that joins an array of CharSequences with a delimiter and applies a masking
   * renderer using the {@link #DEFAULT_MASK}.
   *
   * @param <T> the element type of the CharSequence array
   * @param delimiterCodePoint the character to insert between array elements
   * @return a renderer for arrays of CharSequences
  ...
  public static <T extends CharSequence> Renderer<T[]> delimit(final char delimiter) {
    if (Character.isSurrogate(delimiter))
      throw new IllegalArgumentException(
          "Use code point to specify a delimiter value outside the Basic Multilingual Plane");
    return delimit((int) delimiter);
  }

  /**
   * Returns a renderer that joins an array of CharSequences with the {@link #DEFAULT_DELIMITER }and
   * applies a masking renderer using the {@link #DEFAULT_MASK}.
   *
   * @param <T> the element type of the CharSequence array
   * @return a renderer for arrays of CharSequences
   */
   ...
   */
  public static <T extends CharSequence> Renderer<T[]> delimit(
      final Renderer<CharSequence> renderer, final char delimiter) {
    if (Character.isSurrogate(delimiter))
      throw new IllegalArgumentException(
          "Use code point to specify a delimiter value outside the Basic Multilingual Plane");
   ...
   */
  public static <T extends CharSequence> Renderer<T[]> delimit(
      final Renderer<CharSequence> renderer) {
   ...
   * @return a renderer for arrays of CharSequences
   */
  public static <T extends CharSequence> Renderer<T[]> concatenate(
      final Renderer<CharSequence> renderer) {
    if (renderer == null) throw new NullPointerException("Nested renderer is required");
  ...
  /**
   * Computes the number of symbols to redact. If precision &lt; 0, returns half the length (rounded
   * up), otherwise returns length - precision.
   *
